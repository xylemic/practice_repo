...
products.forEach(({ name, id, price, category }) => {
  dessertCards.innerHTML += `
    <div class="dessert__card">
      <h2>${name}</h2>
      <p class="dessert__price">$${price}</p>
      <p class="product__category">Category: ${category}</p>
      <button id=${id} class="btn add__to__cart__btn">Add to cart</button>
    </div>
  `;
})
...
class ShoppingCart {
  ...
  addItem(id, products) {
    ...
    this.items.forEach(dessert => {
      ...
      // using the id of the current dessert as your propert
      // update the value of the property to be the current value plus one
    ...
    const currentProductCountSpan = document.getElementById(`product-count-for-id${product.id}`);
     ...
     currentProductCount > 1 ? currentProductCountSpan.textContent = `${currentProductCount}x` : productsContainer.innerHTML += `
        <div class="product" id="dessert${id}">
          <p><span class="product__count" id="product-count-for-id${id}"></span> ${name}</p>
          <p>${price}</p>
        </div>
     `;
  }
...
const addToCartBtns = document.getElementsByClassName('add__to__cart__btn');
